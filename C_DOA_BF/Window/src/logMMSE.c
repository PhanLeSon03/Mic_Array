...
int logMMSE_denosie(X_INT16 *sigBuf, X_FLOAT32 *OutBuf, LOGMMSE_VAR *logMMSE)
{
    ...
    X_FLOAT32 *Buf2denoise  = (X_FLOAT32 *)&(logMMSE->Buf2denoise);
    X_FLOAT32 *old_denosebuf= (X_FLOAT32 *)&(logMMSE->old_denosebuf);
    Complex   *FFTIn        = (Complex   *)&(logMMSE->FFTIn);
    X_FLOAT32 *spect        = (X_FLOAT32 *)&(logMMSE->spect);
    X_FLOAT32 *Xk_prev      = (X_FLOAT32 *)&(logMMSE->Xk_prev);
    X_FLOAT32 *noiseMean    = (X_FLOAT32 *)&(logMMSE->noiseMean);
    X_FLOAT32 *sig          = (X_FLOAT32 *)&(logMMSE->sig);
    X_FLOAT32 *gammak       = (X_FLOAT32 *)&(logMMSE->gammak);
    X_FLOAT32 *ksi          = (X_FLOAT32 *)&(logMMSE->ksi);
    X_FLOAT32 *log_sigma_k  = (X_FLOAT32 *)&(logMMSE->log_sigma_k);
    X_FLOAT32 *A            = (X_FLOAT32 *)&(logMMSE->A);
    X_FLOAT32 *vk           = (X_FLOAT32 *)&(logMMSE->vk);
    X_FLOAT32 *ei_vk        = (X_FLOAT32 *)&(logMMSE->ei_vk);
    X_FLOAT32 *hw           = (X_FLOAT32 *)&(logMMSE->hw);
    X_INT32 *Frame_count   = &(logMMSE->nbFrame);

    X_FLOAT32 aa  = 0.98f;
    X_FLOAT32 mu  = 0.98f;
    X_FLOAT32 eta = 0.15f;
    X_FLOAT32 ksi_min = (float)(pow(10,-2.5));
...
int noise_estimate(X_INT16 *sigBuf, LOGMMSE_VAR *logMMSE)
{
    ...
    X_FLOAT32 *spect        = (X_FLOAT32 *)&(logMMSE->spect);
    X_FLOAT32 *noiseMean    = (X_FLOAT32 *)&(logMMSE->noiseMean);
    X_INT32 *Frame_count   = (X_INT32 *)&(logMMSE->nbFrame);
