...
def crsscorr_local(Z, X):
    ...
    for index in range(-PAR.RES,PAR.RES+1):
        ...
        if (index>=0):
            ...
            #Shift = Delay[index%4::4]
            #Out[PAR.RES + index] = sum(x[1-np.floor((index+1)/4):] * Shift[:(Shift.size + np.floor((index+1)/4) -1)])/(Shift.size + np.floor((index+1)/4)-1)
...
def TestDelay(audio_data, idxDir):
    ...
    for i in range(0,8):
        r = crsscorr_local(audio_data[:,idxDir],audio_data[:,i])
        imax = np.argmax(r)
