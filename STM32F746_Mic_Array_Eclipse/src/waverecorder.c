...
uint16_t TestSDO7[8*AUDIO_OUT_BUFFER_SIZE] __attribute__((aligned(64)));
uint16_t TestSDO8[8*AUDIO_OUT_BUFFER_SIZE] __attribute__((aligned(64)));
    ...
    */
	...
	if (Main_stLR==stMIC56)
	{
            ...
            else
            {
               ...
               if ((WaveRec_idxSens6 - WaveRec_idxSens1 != 1)&&(WaveRec_idxSens6==100))
               {
...
void HAL_SPI_MspInit(SPI_HandleTypeDef* hspi)
{
  ...
  /* USER CODE END SPI6_MspInit 0 */
    ...
    */
	...
	HAL_DMA_DeInit(&hdma_spi6_rx);
	HAL_DMA_Init(&hdma_spi6_rx);

	__HAL_LINKDMA(hspi,hdmarx,hdma_spi6_rx);

	HAL_NVIC_SetPriority(DMA2_Stream6_IRQn, INTERRUPT_PRI_AUDIOOUT, 1);
...
/*-------------------------------------------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------------------------------------------*/
    ...
    if ((WaveRecord_flgSDO7Finish==1)&&(WaveRecord_flgSDO8Finish==1))
    {
        ...
        {
	        if(swtSDO7==0x00) /* first half of buffer */
	        {
...
uint8_t CheckEnergyEqual(int16_t * Channel_Ref, int16_t * Channel, int16_t len)
{
    ...
    if ((facEnergy>1.5)||(facEnergy<0.5))
    {
         return 1; 
    }
