...
//  337     hhcd->hc[ch_num].data_pid = HC_PID_SETUP;
        MOVS     R4,#+3
        STRB     R4,[R7, #+9]
        B.N      ??HAL_HCD_HC_SubmitRequest_2
//  338   }
//  339   else
//  340   {
...
//  345   switch(ep_type)
        CMP      R3,#+3
        BHI.N    ??HAL_HCD_HC_SubmitRequest_3
        TBB      [PC, R3]
        DATA
??HAL_HCD_HC_SubmitRequest_0:
        DC8      0x2,0x26,0xD,0x1F
        THUMB
//  346   {
...
??HAL_HCD_HC_SubmitRequest_4:
        CMP      R5,#+1
        BNE.N    ??HAL_HCD_HC_SubmitRequest_3
        CMP      R2,#+0
        BNE.N    ??HAL_HCD_HC_SubmitRequest_3
//  349     {
//  350       if ( length == 0 )
        CMP      R4,#+0
        BNE.N    ??HAL_HCD_HC_SubmitRequest_5
//  351       { /* For Status OUT stage, Length==0, Status Out PID = 1 */
//  352         hhcd->hc[ch_num].toggle_out = 1;
        MOVS     R2,#+1
        ADD      R3,R6,#+64
        STRB     R2,[R3, #+13]
        B.N      ??HAL_HCD_HC_SubmitRequest_5
//  353       }
//  354       
...
//  377         hhcd->hc[ch_num].data_pid = HC_PID_DATA0;    
        MOVS     R3,#+0
        STRB     R3,[R7, #+9]
        B.N      ??HAL_HCD_HC_SubmitRequest_9
//  378       }
//  379       else
//  380       { /* Put the PID 1 */
...
??HAL_HCD_HC_SubmitRequest_7:
        ...
        LDRB     R2,[R2, #+12]
        CMP      R2,#+0
        BEQ.N    ??HAL_HCD_HC_SubmitRequest_12
