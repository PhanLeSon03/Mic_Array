///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V7.50.2.10312/W32 for ARM      13/Feb/2016  11:44:42
...
//        ARM_MATH_CM7
//    List file    =  
//        H:\PhanLeSon\ActivNoise\Microphone\F7\Mic_Array_Project\Mic_Array\Projects\STM32746G\Applications\Audio\Mic_Array\EWARM\STM32F7\List\usbh_msc.s
//
///////////////////////////////////////////////////////////////////////////////
...
//  169     if(phost->device.CfgDesc.Itf_Desc[phost->device.current_interface].Ep_Desc[0].bEndpointAddress & 0x80)
        ...
        LDRB     R0,[R0, #+584]
        LSLS     R0,R0,#+24
        BPL.N    ??USBH_MSC_InterfaceInit_2
//  170     {
...
??USBH_MSC_InterfaceInit_3:
        ...
        LDRB     R0,[R0, #+592]
        LSLS     R0,R0,#+24
        BPL.N    ??USBH_MSC_InterfaceInit_4
//  181     {
...
//  332   switch (MSC_Handle->state)
        LDRB     R0,[R5, #+12]
        CMP      R0,#+0
        BEQ.N    ??USBH_MSC_Process_0
        CMP      R0,#+1
        BEQ.W    ??USBH_MSC_Process_1
...
//  341       switch (MSC_Handle->unit[MSC_Handle->current_lun].state)
        ...
        LDRB     R0,[R0, #+144]
        CMP      R0,#+0
        BEQ.N    ??USBH_MSC_Process_4
        CMP      R0,#+2
        BEQ.W    ??USBH_MSC_Process_5
        CMP      R0,#+3
        BEQ.W    ??USBH_MSC_Process_6
        CMP      R0,#+4
        BEQ.N    ??USBH_MSC_Process_7
        CMP      R0,#+5
        BEQ.W    ??USBH_MSC_Process_8
        CMP      R0,#+8
        BEQ.W    ??USBH_MSC_Process_9
...
//  443              (MSC_Handle->unit[MSC_Handle->current_lun].sense.key == SCSI_SENSE_KEY_NOT_READY) )   
        ...
        LDRB     R0,[R0, #+156]
        CMP      R0,#+6
        BEQ.N    ??USBH_MSC_Process_28
        LDRH     R0,[R5, #+248]
        MOVS     R1,#+52
        MLA      R0,R1,R0,R5
        LDRB     R0,[R0, #+156]
        CMP      R0,#+2
        BNE.N    ??USBH_MSC_Process_29
//  444           {
//  445             
...
//  462           MSC_Handle->unit[MSC_Handle->current_lun].state = MSC_UNRECOVERED_ERROR; 
        ...
        MOVS     R1,#+8
        STRB     R1,[R0, #+144]
        B.N      ??USBH_MSC_Process_31
//  463         }
//  464         else if(scsi_status == USBH_UNRECOVERED_ERROR)
??USBH_MSC_Process_30:
        UXTB     R7,R7            ;; ZeroExt  R7,R7,#+24,#+24
        CMP      R7,#+4
        BNE.N    ??USBH_MSC_Process_31
//  465         {
...
//  530   switch (MSC_Handle->unit[lun].state)
        ...
        BCC.W    ??USBH_MSC_RdWrProcess_1
        CMP      R0,#+7
        BEQ.N    ??USBH_MSC_RdWrProcess_2
        BCS.W    ??USBH_MSC_RdWrProcess_1
//  531   {
//  532  
//  533   case MSC_READ: 
...
//  561         error = USBH_OK;             
        MOVS     R0,#+0
        MOVS     R7,R0
        B.N      ??USBH_MSC_RdWrProcess_9
//  562     }
//  563     else if( scsi_status == USBH_FAIL)
??USBH_MSC_RdWrProcess_8:
        UXTB     R8,R8            ;; ZeroExt  R8,R8,#+24,#+24
        CMP      R8,#+2
        BNE.N    ??USBH_MSC_RdWrProcess_10
//  564     {
...
??USBH_MSC_RdWrProcess_10:
        UXTB     R8,R8            ;; ZeroExt  R8,R8,#+24,#+24
        CMP      R8,#+4
        BNE.N    ??USBH_MSC_RdWrProcess_9
//  568     {
...
??USBH_MSC_RdWrProcess_11:
        UXTB     R8,R8            ;; ZeroExt  R8,R8,#+24,#+24
        CMP      R8,#+2
        BNE.N    ??USBH_MSC_RdWrProcess_12
//  591     {
//  592       USBH_UsrLog ("MSC Device NOT ready");
        ...
        ADR.N    R0,??DataTable2  ;; "\n"
          CFI FunCall printf
        BL       printf
        B.N      ??USBH_MSC_RdWrProcess_13
//  593     }
//  594     else if(scsi_status == USBH_UNRECOVERED_ERROR)
??USBH_MSC_RdWrProcess_12:
        UXTB     R8,R8            ;; ZeroExt  R8,R8,#+24,#+24
        CMP      R8,#+4
        BNE.N    ??USBH_MSC_RdWrProcess_13
//  595     {
...
//  623     return (MSC_Handle->state == MSC_IDLE);
        ...
        MOVS     R0,#+1
        B.N      ??USBH_MSC_IsReady_2
??USBH_MSC_IsReady_1:
        MOVS     R0,#+0
??USBH_MSC_IsReady_2:
        UXTB     R0,R0            ;; ZeroExt  R0,R0,#+24,#+24
        B.N      ??USBH_MSC_IsReady_3
//  624   }
//  625   else
//  626   {
//  627     return 0;
...
//  643     return  MSC_Handle->max_lun;
        ...
        B.N      ??USBH_MSC_GetMaxLUN_1
//  644   }  
//  645   return 0xFF;
??USBH_MSC_GetMaxLUN_0:
        MOVS     R0,#-1
??USBH_MSC_GetMaxLUN_1:
        BX       LR               ;; return
//  646 }
...
//  661     return (MSC_Handle->unit[lun].error == MSC_OK);
        ...
        LDRB     R0,[R0, #+145]
        CMP      R0,#+0
        BNE.N    ??USBH_MSC_UnitIsReady_1
        MOVS     R0,#+1
        B.N      ??USBH_MSC_UnitIsReady_2
??USBH_MSC_UnitIsReady_1:
        MOVS     R0,#+0
??USBH_MSC_UnitIsReady_2:
        UXTB     R0,R0            ;; ZeroExt  R0,R0,#+24,#+24
        B.N      ??USBH_MSC_UnitIsReady_3
//  662   }
//  663   else
//  664   {
//  665     return 0;
