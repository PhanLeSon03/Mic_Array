...
//  225 //extern arm_rfft_fast_instance_f32 S1,S2,S3,S4,IS;
//  226 
//  227 /*------------------------------------------------------------------------------------------------------------*/
//  228 /*--------------------- VARIABLES-----------------------------------------------------------------------------*/
//  229 
...
//  235 
        ...
        SECTION `.bss`:DATA:REORDER:NOROOT(2)
        DATA
//  236 float vDataIn1_FFT[AUDIO_OUT_BUFFER_SIZE];
vDataIn1_FFT:
        DS8 4096
//  237 float vDataIn2_FFT[AUDIO_OUT_BUFFER_SIZE];
vDataIn2_FFT:
        DS8 4096
//  238 float vDataIn2_FFT_CJ[AUDIO_OUT_BUFFER_SIZE];
//  239 float vDataIn_FFT[AUDIO_OUT_BUFFER_SIZE];
vDataIn_FFT:
        DS8 4096
vDataIn2_FFT_CJ:
        DS8 4096
//  240 float vDataOut[AUDIO_OUT_BUFFER_SIZE];
vDataOut:
        DS8 4096
//  241 float vDataIn[AUDIO_OUT_BUFFER_SIZE];
vDataIn:
        DS8 4096
//  242 
//  243 /*------------------------------------------------------------------------------------------------------------*/
...
//  279 		    xOfn -= IN[2*m+1]*arm_sin_f32(2*PI*cycles*(float)m*(float)n/(float)N);
        ...
        VMLS.F32 S17,S1,S0
//  280 
//  281                     xOfn_m += IN[2*m]*arm_cos_f32(2*PI*cycles*(float)m*(float)n/(float)N);
        VMOV.F32 S0,S20
          CFI FunCall arm_cos_f32
        BL       arm_cos_f32
        VLDR     S1,[R7, #0]
        VMLA.F32 S18,S1,S0
//  282                     xOfn_m += IN[2*m+1]*arm_sin_f32(2*PI*cycles*(float)m*(float)n/(float)N);
        VMOV.F32 S0,S20
          CFI FunCall arm_sin_f32
        BL       arm_sin_f32
        VLDR     S1,[R7, #+4]
...
//  344 	for(uint16_t i=0; i<Size; i++)
        ...
        LDRSH    R5,[R4, #+0]
        BEQ.N    ??LowPass_0
        LDR.W    R9,??DataTable14  ;; 0xffff8000
        MOVW     R7,#+32767
//  345 	{
...
//  647     for (uint32_t currentSample = 0; currentSample < 16; currentSample++) // go for all the output sample
        MOVS     R3,#+16
        LDR.W    R8,??DataTable14  ;; 0xffff8000
        MOVW     R9,#+32767
...
//  972 
//  973           /* Revert FFT*/
//  974           arm_rfft_f32(&IS, (float *)bufferFFTSum, (float *)&fbufferOut[iFrm*lenFFT]);
        MUL      R0,R6,R7
        LDR      R1,[SP, #+16]
        ADDS     R7,R7,#+1
        UXTH     R7,R7
...
// 1133   float maxe = -log2(1.0/(float)N); /* max entropy */
        ...
        MOV      R3,R1
        MOVS     R0,#+0
        LDR.N    R1,??DataTable16_13  ;; 0x3ff00000
          CFI FunCall __aeabi_ddiv
        BL       __aeabi_ddiv
        VMOV     D0,R0,R1
        MOV      R0,R4
