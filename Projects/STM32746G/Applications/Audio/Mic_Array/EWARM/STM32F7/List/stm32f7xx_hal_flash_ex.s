///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V7.50.2.10312/W32 for ARM      22/Dec/2015  15:52:41
// Copyright 1999-2015 IAR Systems AB.
//
//    Cpu mode     =  thumb
//    Endian       =  little
//    Source file  =  
//        D:\sop1hc\Github\data\Mic_Array_V00\Mic_Array\Drivers\STM32F7xx_HAL_Driver\Src\stm32f7xx_hal_flash_ex.c
//    Command line =  
//        D:\sop1hc\Github\data\Mic_Array_V00\Mic_Array\Drivers\STM32F7xx_HAL_Driver\Src\stm32f7xx_hal_flash_ex.c
...
//        ARM_MATH_CM7
//    List file    =  
//        D:\sop1hc\Github\data\Mic_Array_V00\Mic_Array\Projects\STM32746G\Applications\Audio\Mic_Array\EWARM\STM32F7\List\stm32f7xx_hal_flash_ex.s
//
///////////////////////////////////////////////////////////////////////////////
...
//  187     if(pEraseInit->TypeErase == FLASH_TYPEERASE_MASSERASE)
        LDR.N    R7,??DataTable7_1  ;; 0x40023c10
        STR      R1,[R9, #+0]
        LDR      R1,[R4, #+0]
        CMP      R1,#+1
...
//  249   __HAL_FLASH_ENABLE_IT(FLASH_IT_EOP);
        LDR.N    R2,??DataTable7_2  ;; 0x40023c0c
        LDR      R3,[R2, #+4]
        ORR      R3,R3,#0x1000000
        STR      R3,[R2, #+4]
//  250   
...
//  301   if((pOBInit->OptionType & OPTIONBYTE_WRP) == OPTIONBYTE_WRP)
        LDRB     R1,[R4, #+0]
        LDR.N    R6,??DataTable7_3  ;; 0x40023c14
        MOVW     R8,#+50000
        LSLS     R1,R1,#+31
...
//  355 }
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI (cfiCond4) Function HAL_FLASHEx_Erase
          ...
          CFI (cfiCond5) Function HAL_FLASHEx_Erase
          ...
          CFI (cfiCond6) Function HAL_FLASHEx_OBProgram
          CFI (cfiCond6) Conditional ??CrossCallReturnLabel_0
          CFI (cfiCond6) R4 Frame(CFA, -24)
          CFI (cfiCond6) R5 Frame(CFA, -20)
          CFI (cfiCond6) R6 Frame(CFA, -16)
          CFI (cfiCond6) R7 Frame(CFA, -12)
          CFI (cfiCond6) R8 Frame(CFA, -8)
...
?Subroutine1:
        MOV      R0,R8
          CFI (cfiCond3) FunCall HAL_FLASHEx_Erase FLASH_WaitForLastOperation
          CFI (cfiCond3) FunCall HAL_FLASHEx_Erase FLASH_WaitForLastOperation
          CFI (cfiCond3) FunCall HAL_FLASHEx_Erase FLASH_WaitForLastOperation
          CFI (cfiCond6) FunCall HAL_FLASHEx_OBProgram FLASH_WaitForLastOperation
...
//  370   pOBInit->WRPSector = FLASH_OB_GetWRP();
        LDR.N    R0,??DataTable7_3  ;; 0x40023c14
        LDR      R1,[R0, #+0]
        AND      R1,R1,#0xFF0000
        STR      R1,[R4, #+8]
//  371 
...
//  376   pOBInit->USERConfig = FLASH_OB_GetUser();
        LDR.N    R2,??DataTable7_4  ;; 0xc00000f0
        ITTTT    NE 
        LDRNE    R1,[R0, #+0]
        UBFXNE   R1,R1,#+8,#+8
        CMPNE    R1,#+204
...
FLASH_MassErase:
        LDR.N    R0,??DataTable7_1  ;; 0x40023c10
        LDR      R1,[R0, #+0]
        BIC      R1,R1,#0x300
        STR      R1,[R0, #+0]
...
??FLASH_Erase_Sector_0:
        LDR.N    R1,??DataTable7_1  ;; 0x40023c10
...
//  625   if(status == HAL_OK)
        CBNZ.N   R0,??FLASH_OB_UserConfig_0
//  626   {
...
//  633     MODIFY_REG(FLASH->OPTCR, useroptionmask, useroptionvalue);
        LDR.N    R1,??DataTable7_3  ;; 0x40023c14
        LDR.N    R3,??DataTable7_5  ;; 0x3fffff0f
        LDR      R2,[R1, #+0]
        ANDS     R2,R3,R2
        ORRS     R2,R5,R2
        ORRS     R2,R4,R2
...
//  691   if(status == HAL_OK)
        CBNZ.N   R0,??FLASH_OB_BootAddressConfig_0
//  692   {
//  693     if(BootOption == OPTIONBYTE_BOOTADDR_0)
        LDR.N    R1,??DataTable7_6  ;; 0x40023c18
        CMP      R4,#+16
        LDR      R2,[R1, #+0]
        ITTE     EQ 
        LSREQ    R2,R2,#+16
        ORREQ    R2,R5,R2, LSL #+16
        PKHBTNE  R2,R2,R5, LSL #+16
//  694     {			
